...
test.beforeEach(t => availablePort()
    .then(port => {
      t.context.port = port
      return server.start({
        environment: 'test',
...
test('the home page renders well', t => get(`http://localhost:${t.context.port}/`)
    .then(response => {
...
const pipeToString = (encoding, stream) => new Promise((resolve, reject) => {
  let string = ''
  stream.on('data', data => {
